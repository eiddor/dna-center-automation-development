...
def main():
    ...
    for lan_pool in project_data['ip_lan_pool']:
        ip_lan_pool_name = lan_pool['name']
        ip_lan_pool_type = lan_pool['type']
        ip_lan_pool_cidr = lan_pool['subnet']
        ip_lan_pool_address_space = lan_pool['address_family']
        # create an IP transit pool for site_hierarchy
        print('\n\nCreating the IP LAN pool: ', ip_lan_pool_cidr)
        ip_lan_pool_subnet = ip_lan_pool_cidr.split('/')[0]
        ip_lan_pool_mask = int(ip_lan_pool_cidr.split('/')[1])
        lan_pool_payload = {
        ...
        building_name = lan_pool['building']
        site_hierarchy = 'Global/' + area_name + '/' + building_name
        response = dnac_api.sites.get_site(name=site_hierarchy)
        site_id = response['response'][0]['id']
